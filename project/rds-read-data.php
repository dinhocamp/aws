...
<?php
  ...
  $conn = mysqli_connect($RDS_URL, $RDS_user, $RDS_pwd, $RDS_DB) or die(mysqli_error());
...
if ( $mode=="add")
 ...
 if ( $mode=="added")
 {
 mysqli_query ($conn,"INSERT INTO address (lastname, firstname, phone, email) VALUES ('$lastname', '$firstname', '$phone', '$email')");
 }
...
if ( $mode=="edit")
 ...
 if ( $mode=="edited")
 {
 mysqi_query ($conn,"UPDATE address SET lastname = '$lastname', firstname = '$firstname', phone = '$phone', email = '$email' WHERE id = $id");
...
try{
 $data = mysqli_query($conn,"SELECT * FROM address ORDER BY lastname ASC") ;

}
catch (Exception $e){
$mysql_command = "cd /var/www/html/ ; mysql --user=main --host=database-1.cmnpw0tf0dvq.us-east-1.rds.amazonaws.com --password=lab-password lab < ./sql/addressbook.sql";
echo shell_exec($mysql_command);
}
$data = mysqli_query($conn,"SELECT * FROM address ORDER BY lastname");
